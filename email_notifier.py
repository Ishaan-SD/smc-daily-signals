...
class EmailNotifier:
    ...
    def send_summary(self, signals: list, interval: str = "1d"):
        """
        Dispatches a single consolidated HTML email alert.
        If no signals are triggered, it sends a heartbeat 'All Clear' message.
        """
        msg = MIMEMultipart("alternative")
        msg["From"] = self.sender_email
        msg["To"] = self.receiver_email

        # --- SCENARIO 1: No Signals (Heartbeat Email) ---
        if not signals:
            msg["Subject"] = f"✅ SMC Market Scan: All Clear ({interval})"
            html_content = f"""
            <html>
              <body style="font-family: Arial, sans-serif; color: #333;">
                <h2>✅ Daily SMC Market Scan ({interval})</h2>
                <p>The automated market scan completed successfully, but <strong>no new SMC signals</strong> were detected at today's market close.</p>
                <p style="margin-top: 30px; font-size: 12px; color: #888;">Automated by your Quantitative Trading Backend.</p>
              </body>
            </html>
            """
            
        # --- SCENARIO 2: Signals Found (Table Email) ---
        else:
